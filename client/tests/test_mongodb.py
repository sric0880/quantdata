...
def test_get_trade_cal():
    with qd.mongo_connect(host) as client:
        ...
        c = qd.mongo_get_trade_cal(db, start_date=datetime.datetime(2024, 8, 19, 23), end_date=datetime.datetime(2024, 9, 19))
        df = pd.DataFrame(c)
        logging.info(df)
        ...
        last_dt = qd.mongo_get_last_trade_dt(db, datetime.datetime(2023, 4, 10, 23, 0, 0))
